...
from utils.utils import (
    ...
    get_freq_of_items_in_list
)


class StyleFeatures(object):
    '''Retrieve features of writing style. 
    ...
    def __init__(self, posts):
        ...
        self.posts_wo_links = self.remove_links_from_text(self.posts)
        self.all_punctuation_in_text = [
            char for char in self.posts_wo_links if char in punctuation
        ]
    ...
    def get_posts_with_periods_at_ends(self):
        '''This adds periods to the end of posts which are missing punctuation 
        at the end. This is so TextBlob will be able to extract sentences from 
        the text properly.
        '''
    ...
    def get_sentences(self):
        '''Extract all sentences from a string of text.
    ...
    def separate_links_from_words(self):
        '''Extract all unique url links from list of words.
    ...
    def add_parts_of_speech_frequencies_to_stylometry_markers(self):
        ...
        pos_abbrs = {
            'adjs_per_word': 'JJ',
            'interjections_per_word': 'UH', 
            'proper_nouns_per_word': 'NNP', 
            'nouns_per_word': 'NN',
            'personal_pronouns_per_word': 'PRP', 
            'prepositions_per_word': 'IN',
            'determiners_per_word': 'DT', 
            'adverbs_per_word': 'RB', 
            'verbs_per_word': 'VB',
        }

        for key, pos_abbr in pos_abbrs.items():
            self.stylometry_markers[key] = (
                pos_tags.count(pos_abbr) / self.num_of_non_link_words
            )

    def add_frequencies_of_punctuation_tokens_per_sentence(self):
        for punct in freq_punctuation_per_sentence:
            self.stylometry_markers[punct + '_per_sentence'] = (
                get_freq_of_items_in_list(
    ...
    def add_frequencies_of_punctuation_tokens_per_word(self):
        for punct in freq_punctuation_per_word:
            self.stylometry_markers[punct + '_per_word'] = (
                get_freq_of_items_in_list(
                    self.all_punctuation_in_text, [punct], self.num_of_words
                ) * 1000
            )
    ...
    def add_frequencies_of_tokens_to_stylometry_markers(self):
        for key, items in freq_token_types_per_word.items():
            self.stylometry_markers[key] = get_freq_of_items_in_list(
                self.words, items, self.num_of_words
            ) * 1000

    def add_frequencies_of_link_types_to_stylometry_markers(self):
        num_of_links = len(self.links)
        self.stylometry_markers['links_per_post'] = (
            num_of_links / self.num_of_posts
        ) * 100

        for search_term in LINK_TYPES:
            self.stylometry_markers[search_term + '_per_post'] = (
                get_freq_of_characters_in_text_in_list(
                    self.links, [search_term], self.num_of_posts
                )
            ) * 100

        self.stylometry_markers['images_per_post'] = (
            get_freq_of_characters_in_text_in_list(
                self.links, IMAGE_TERMS, self.num_of_posts
            )
        ) * 100

    def add_avg_lens_to_stylometry_markers(self):
        self.stylometry_markers['avg_words_per_post'] = (
            self.num_of_words / self.num_of_posts
        )
        self.stylometry_markers['word_diversity'] = (
            len(
                [set(word.lower() for word in self.non_link_words)]
            ) / self.num_of_non_link_words
        )
        self.stylometry_markers['avg_word_length'] = np.mean(
            [len(word) for word in self.non_link_words]
        )

        words_per_sentences = [
            len(sentence.split()) for sentence in self.sentences
        ]
    ...
    def add_sentiment_analysis_to_stylometry_markers(self):
        sentiment = self.textblob_posts.sentiment
        self.stylometry_markers['polarity'] = sentiment.polarity
        self.stylometry_markers['subjectivity'] = sentiment.subjectivity
    ...
    def add_frequency_of_emojis_and_emoticons_to_stylometry_markers(self):
        # NOTE: .sentences removes ellipses that come at the end of sentences. 
        # fix this.
        ...
        smilies = [':)', ':D', ': )', ':-)']
        self.stylometry_markers['smilies_per_sentence'] = sum(
            get_count_of_characters_in_text(self.posts_wo_links, smilie) 
            for smilie in smilies
        ) / self.num_of_sentences

        self.stylometry_markers['hashtags_per_sentence'] = (
            get_count_of_characters_in_text(self.posts_wo_links, '#')
        ) / self.num_of_sentences
